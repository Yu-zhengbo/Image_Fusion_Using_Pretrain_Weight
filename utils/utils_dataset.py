...
class FusionDataset(Dataset):
    def __init__(self, root, is_train=True, with_det=False,img_size=640):
        self.root = root
        ...
        self.inf_transform = Compose([Grayscale(3), Resize((img_size, img_size)), ToTensor()])

    def __getitem__(self, index):
        vis = Image.open(
            os.path.join(self.visible_path, self.visible_img_list[index])
        ).convert("RGB")
        inf = Image.open(
            os.path.join(self.infrared_path, self.visible_img_list[index])
        ).convert("L")
