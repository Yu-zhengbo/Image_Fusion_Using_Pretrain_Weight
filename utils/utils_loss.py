...
class GeneratorLoss(nn.Module):
    ...
    def forward(self, fusion, visible, infrared):
        ...
        for feature_model in [self.local_feature, self.globa_feature]:
            for f, v, i in zip(
                feature_model(fusion), feature_model(visible), feature_model(infrared)
            ):
                ...
                v_grad = F.pad(
                    get_gaussian_kernel_temp(LaplacianLoss_temp(v)), [3, 3, 3, 3]
                )
                i_grad = F.pad(
                    get_gaussian_kernel_temp(LaplacianLoss_temp(i)), [3, 3, 3, 3]
                )
                f_grad = F.pad(
                    get_gaussian_kernel_temp(LaplacianLoss_temp(f)), [3, 3, 3, 3]
                )
        ...
        return vis_loss + inf_loss + ssim_loss + tv_loss #+ 1*torch.mean(fusion)
...
class LaplacianLoss(nn.Module):
    def __init__(self, channels=3):
        ...
        self.laplacian_filter = nn.Conv2d(
            in_channels=channels,
            out_channels=channels,
            kernel_size=3,
            groups=channels,
...
class local_feature_information(nn.Module):
    ...
    def forward(self, x):
        ...
        x4 = self.layer4(x3)
        return (
            self.minmax_scaler(x1),
            self.minmax_scaler(x2),
            self.minmax_scaler(x3),
            self.minmax_scaler(x4),
        )
