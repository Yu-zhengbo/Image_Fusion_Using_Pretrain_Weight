...
if __name__ == "__main__":
    ...
    #   结果输出的文件夹，默认为map_out
    # -------------------------------------------------------#
    map_out_path = "./det/map_out"

    JPEGImages = "tardal"

    image_ids = (
        open(os.path.join(VOCdevkit_path, "ImageSets/test.txt")).read().strip().split()
    )
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path = os.path.join(
                VOCdevkit_path, "%s/" % JPEGImages + image_id + ".png"
            )
            image = Image.open(image_path)
            if map_vis:
                image.save(
                    os.path.join(map_out_path, "images-optional/" + image_id + ".jpg")
                )
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            with open(
                ...
                root = ET.parse(
                    os.path.join(VOCdevkit_path, "ann/" + image_id + ".xml")
                ).getroot()
